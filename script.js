...
//
//
function redirectToPreviewPage(index) {
    const selectedBook = booksData[index];
    urlToPreview = `preview.html?title=${selectedBook.title}&author=${selectedBook.author}&price=${selectedBook.price}&description=${selectedBook.description}&image=${selectedBook.image}&rentPerDay=${selectedBook.rentPerDay}&stores=${selectedBook.stores}&daysAllowed=${selectedBook.daysAllowed}&copies=${selectedBook.copiesAvailable}`
    window.location.href = urlToPreview;
  }
...
function addCardEventListeners() {
    ...
    cards.forEach((card, index) => {
      card.addEventListener('click', () => {
        ...
        redirectToPreviewPage(index); // Pass the index of the clicked card
      });
    });
  }
  
  // Call the function to add event listeners when the page loads
  window.addEventListener("DOMContentLoaded", addCardEventListeners);
